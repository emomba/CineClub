...
export default function Landing() {
  ...
  const enterClub = async () => {
    ...
    try {
      ...
      if (ok) { setLocation("/home"); return; }
    } catch {}
    // Session restore failed — go to login with username pre-filled
      ...
      <div className="z-10 text-center max-w-3xl px-6">
        ...
        {hasSaved ? (
          ...
          <button
            onClick={enterClub}
            disabled={entering}
            className="relative group inline-flex items-center justify-center gap-2 px-12 py-4 rounded-2xl text-lg font-bold text-black transition-all duration-200 hover:scale-[1.03] active:scale-[0.98] disabled:opacity-80 disabled:cursor-wait disabled:scale-100"
            style={{
          ...
          >
            {entering && <Loader2 size={18} className="animate-spin" />}
