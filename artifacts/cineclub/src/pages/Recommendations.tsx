...
export default function Recommendations() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 max-w-4xl mx-auto">
        ...
        {[...Array(4)].map((_, i) => <Skeleton key={i} className="h-40 w-full bg-[#111] rounded-2xl" />)}
      </div>
    );
  }
  ...
  return (
    <PageTransition className="max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-4xl font-bold tracking-tight mb-2">{t("inbox")}</h1>
        <p className="text-gray-400">{t("inboxSubtitle")}</p>
      </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="text-sm text-gray-400">
                    <span className="font-bold text-white">{rec.fromUser.displayName || rec.fromUser.username}</span> {t("recommendedBy")}
                  </div>
