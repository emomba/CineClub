...
import { Bell, Check, Users, MessageSquare } from "lucide-react";
      ...
      {notifications && notifications.length > 0 ? (
        <div className="space-y-2 bg-[#111] border border-gray-800 rounded-3xl p-2">
          {notifications.map(notif => (
            <div
              key={notif.id}
              className={`p-4 rounded-2xl flex items-start gap-4 transition-colors cursor-pointer ${notif.read ? 'opacity-60' : 'bg-gray-800/30'}`}
              onClick={() => !notif.read && handleMarkRead(notif.id)}
            >
              <div className="mt-1 shrink-0 p-2 bg-black rounded-full border border-gray-800">{getIcon(notif.type)}</div>
              ...
              {!notif.read && <div className="w-2 h-2 rounded-full bg-amber-500 shrink-0 mt-2 shadow-[0_0_8px_rgba(245,158,11,0.8)]" />}
            </div>
          ))}
        </div>
      ) : (
