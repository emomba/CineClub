...
export function MovieCard({
  ...
  const rating = typeof movie.voteAverage === "number" ? movie.voteAverage : parseFloat(String(movie.voteAverage ?? "0"));
          ...
          <div className="flex items-center justify-between mt-0.5 text-xs text-gray-400">
            ...
            {showRating && rating > 0 && (
              <div className="flex items-center gap-0.5">
                ...
                <span className="text-amber-400 font-semibold tabular-nums">{rating.toFixed(1)}</span>
              </div>
            )}
          </div>
        </div>
